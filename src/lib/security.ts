...
export async function requireAdmin() {
  ...
  return getDb().prepare("SELECT username FROM admin_users WHERE username = ?").get(username);
}
...
export function verifyAgentToken(authHeader: string | null) {
  const token = authHeader?.replace(/^Bearer\s+/i, "");
  if (!token) return false;
  const rows = getDb().prepare("SELECT token_hash FROM agent_tokens").all() as Array<{ token_hash: string }>;
  return rows.some((row) => verifySecret(token, row.token_hash));
}
